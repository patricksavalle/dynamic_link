	...
	function __construct()
	{
		...
		if (empty($metadata)) {
			...
			if (stripos($url, "youtube.com")) {
				// Could not construct an XPATH expression that worked on the Youtube HTML. Be my guest! Until than...
				// see https://stackoverflow.com/questions/10896233/how-can-i-retrieve-youtube-video-details-from-video-url-using-php
				parse_str(parse_url($url, PHP_URL_QUERY), $query);
				$s_get = @file_get_contents('https://youtube.com/get_video_info?video_id=' . $query['v']);
				parse_str($s_get, $m_get);
				if (empty($m_get['player_response'])) {
					$metadata["crawled"] = false;
				} else {
					$json = json_decode($m_get['player_response'])->microformat->playerMicroformatRenderer;
					$metadata["crawled"] = true;
					$metadata['link_url'] = $url;
					$metadata['link_title'] = $json->title->simpleText;
					$metadata['link_description'] = $json->description->simpleText;
					$metadata['link_image'] = $json->thumbnail->thumbnails[0]->url;
					$metadata['link_domain'] = parse_url($metadata['link_url'], PHP_URL_HOST);
					$metadata['link_site_name'] = "Youtube";
				}
			}
			else {
